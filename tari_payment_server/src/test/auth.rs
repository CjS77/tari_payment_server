...
use tari_payment_engine::{AuthApi, AuthApiError};
...
async fn login_without_headers() {
    ...
    let func = configure_app(config.clone(), Ok(None));
...
async fn login_with_invalid_header() {
    let _ = env_logger::try_init().ok();
    let (status, body, _) = post_request("made up nonsense", Ok(None)).await;
...
async fn login_with_invalid_signature() {
    ...
    let (status, body, _) = post_request(token, Ok(None)).await;
...
async fn login_with_valid_token() {
    ...
    let (status, s, config) = post_request(token, Ok(Some(42))).await;
...
async fn login_with_disallowed_roles() {
    let token = "eyJhbGciOiJSaXN0cmV0dG8yNTYiLCJ0eXAiOiJKV1QifQ.eyJhZGRyZXNzIjp7Im5ldHdvcmsiOiJtYWlubmV0IiwicHVibGljX2tleSI6IjEyYTI1MDRhNzhmMDg5MzBjMmQzMzU3MDhmYWU4MDY5NmIyMTdkMjNiZDJkNDczZTEyN2Q4ZjVhMzBlMjgxNjUifSwibm9uY2UiOjE3MTE0NDg2NDksImRlc2lyZWRfcm9sZXMiOlsidXNlciIsIndyaXRlIl19.gm2Z6FxNyMmLT_dcEGu_iH9_wBm029OY_eqw__hZ0yXpa0ccVeBbF1lTfYU5xEhmGtQXtwhOjC8l2SUm9QB8CQ";
    let (status, body, _) = post_request(token, Err(AuthApiError::RoleNotAllowed(Role::Write))).await;
    assert_eq!(status.as_u16(), StatusCode::FORBIDDEN.as_u16());
    assert_eq!(body, r#"{"error":"Authentication Error. Insufficient Permissions. User requested a role that is not allowed: write"}"#);
}

#[actix_web::test]
async fn login_with_no_preexisting_user_account() {
    let token = "eyJhbGciOiJSaXN0cmV0dG8yNTYiLCJ0eXAiOiJKV1QifQ.eyJhZGRyZXNzIjp7Im5ldHdvcmsiOiJtYWlubmV0IiwicHVibGljX2tleSI6IjEyYTI1MDRhNzhmMDg5MzBjMmQzMzU3MDhmYWU4MDY5NmIyMTdkMjNiZDJkNDczZTEyN2Q4ZjVhMzBlMjgxNjUifSwibm9uY2UiOjE3MTE0NDg2NDksImRlc2lyZWRfcm9sZXMiOlsidXNlciIsIndyaXRlIl19.gm2Z6FxNyMmLT_dcEGu_iH9_wBm029OY_eqw__hZ0yXpa0ccVeBbF1lTfYU5xEhmGtQXtwhOjC8l2SUm9QB8CQ";
    let (status, body, _) = post_request(token, Err(AuthApiError::PubkeyNotFound)).await;
    assert_eq!(status.as_u16(), StatusCode::FORBIDDEN.as_u16());
...
async fn login_with_invalid_nonce() {
    ...
    assert_eq!(status.as_u16(), StatusCode::UNAUTHORIZED.as_u16());
    assert_eq!(body, r#"{"error":"Authentication Error. Login token signature is invalid. Nonce is not strictly increasing."}"#);
}


fn configure_app(config: AuthConfig, update_nonce_result: Result<Option<i64>, AuthApiError>) -> impl FnOnce(&mut ServiceConfig) {
    move |cfg| {
        let mut auth_manager = MockAuthManager::new();
        auth_manager
            .expect_update_nonce_for_address()
            .return_const(update_nonce_result);
...
async fn post_request(auth_header: &str, update_nonce_result: Result<Option<i64>, AuthApiError>) -> (StatusCode, String, AuthConfig) {
