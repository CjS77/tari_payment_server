...
async fn receive_wallet_payment(
    world: &mut ShopifyWorld,
    txid: String,
    pubkey: String,
    amount: i64,
...
#[when(
    expr = "I receive a wallet payment with txid [{word}] from '{word}' for {int} XTR and memo {string}"
)]
async fn wallet_payment_with_memo(
    world: &mut ShopifyWorld,
    txid: String,
    pubkey: String,
    amount: i64,
...
async fn fetch_customer_account(world: &mut ShopifyWorld, cust_id: &str) -> Option<UserAccount> {
    let db = world.api().db();
    db.fetch_user_account_for_customer_id(cust_id)
        .await
...
async fn check_account_exists_for_pubkey(world: &mut ShopifyWorld, pubkey: String) {
    ...
    let account = db
        .fetch_user_account_for_pubkey(&pubkey)
        .await
        .expect("Error fetching account");
    assert!(account.is_some(), "Account does not exist");
}
...
async fn check_account_total_orders(world: &mut ShopifyWorld, customer_id: String, value: i64) {
    let account = fetch_customer_account(world, &customer_id)
        .await
        .expect("Account for {customer_id} does not exist");
    assert_eq!(
        account.total_orders,
        MicroTari::from(value * 1_000_000),
        "Total orders is incorrect"
    );
}
...
async fn check_customer_balance(world: &mut ShopifyWorld, customer_id: String, value: i64) {
    ...
    assert_eq!(
        ...
        MicroTari::from(value * 1_000_000),
        "Current balance is incorrect"
    );
}

async fn fetch_account_for_pubkey(world: &mut ShopifyWorld, pubkey: String) -> Option<UserAccount> {
    let db = world.api().db();
    let pubkey = TariAddress::from_str(&pubkey).expect("Not a valid Tari address. {pubkey}");
    db.fetch_user_account_for_pubkey(&pubkey)
        .await
        .expect("Error fetching account {pubkey}")
}
...
async fn check_account_total_received(world: &mut ShopifyWorld, pubkey: String, value: i64) {
    let account = fetch_account_for_pubkey(world, pubkey)
        .await
        .expect("Account {pubkey} does not exist");
    assert_eq!(
        account.total_received,
        MicroTari::from(value * 1_000_000),
        "Total received is incorrect"
