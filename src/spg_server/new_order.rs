...
impl OrderBuilder {
    ...
    pub fn random_order() -> FreshOrder {
        OrderBuilder::new().build()
    }

    pub fn custom_order(note: String, price: &str) -> FreshOrder {
...
mod test {
    ...
    fn deserialize_new_order() {
        ...
        let order: FreshOrder = serde_json::from_str(order).unwrap();
